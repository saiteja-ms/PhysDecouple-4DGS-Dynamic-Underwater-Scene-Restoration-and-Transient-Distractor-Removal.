...
class Deformation(nn.Module):
    ...
    def forward_dynamic(self,rays_pts_emb, scales_emb, rotations_emb, opacity_emb, shs_emb, time_feature, time_emb):
        ...
        # [FIX 2] Sanitize hidden activations before deform heads
        hidden = torch.nan_to_num(hidden)
        hidden = hidden.clamp(-10.0, 10.0)
        ...
        if self.args.no_dx:
            ...
            dx = torch.nan_to_num(dx).clamp(-2.0, 2.0)   # [FIX 3]
        ...
        if self.args.no_ds :
            ...
            ds = torch.nan_to_num(ds).clamp(-2.0, 2.0)   # [FIX 3] clamp scale deltas
        ...
        if self.args.no_dr :
            ...
            dr = torch.nan_to_num(dr).clamp(-1.0, 1.0)   # [FIX 3]
        ...
        if self.args.no_do :
            ...
            do = torch.nan_to_num(do).clamp(-2.0, 2.0)   # [FIX 3]
        ...
        if self.args.no_dshs:
            ...
            dshs = torch.nan_to_num(dshs).clamp(-2.0, 2.0)   # [FIX 3]

            shs = torch.zeros_like(shs_emb)
            # breakpoint()
            shs = shs_emb*mask.unsqueeze(-1) + dshs

        # [FIX 1] Clamp final deformation outputs
        pts = pts.clamp(-50.0, 50.0)
        scales = scales.clamp(-5.0, 5.0)
        rotations = torch.nan_to_num(rotations)
        opacity = opacity.clamp(-10.0, 10.0)
