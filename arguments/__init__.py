...
class ModelHiddenParams(ParamGroup):
    def __init__(self, parser):
        ...
        self.bounds = 1.6 
        self.plane_tv_weight = 0.01 # TV loss of spatial grid
        self.time_smoothness_weight = 0.01 # TV loss of temporal grid
        self.l1_time_planes = 0.01  # TV loss of temporal grid
        self.kplanes_config = {
                             'grid_dimensions': 2,
...
def get_combined_args(parser : ArgumentParser):
    ...
    cfg_has_values = bool(vars(args_cfgfile))
    for k,v in vars(args_cmdline).items():
        if v is not None and (not cfg_has_values or k in explicit_dests):
            merged_dict[k] = v
