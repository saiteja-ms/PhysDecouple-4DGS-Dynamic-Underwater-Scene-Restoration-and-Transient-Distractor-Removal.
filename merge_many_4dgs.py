...
if args.configs1:
    ...
    spec = importlib.util.spec_from_file_location("_uw_cfg1", args.configs1)
    mod1 = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod1)
    for cls_name in ["ModelParams", "OptimizationParams", "PipelineParams"]:
        if hasattr(mod1, cls_name):
            cls_obj = getattr(mod1, cls_name)
            for k, v in vars(cls_obj).items():
                if not k.startswith("__"):
                    if not hasattr(args1, k) or getattr(args1, k) is None:
                        setattr(args1, k, v)
# breakpoint()
if args2.configs2:
    import importlib.util
    from utils.params_utils import merge_hparams
    spec = importlib.util.spec_from_file_location("_uw_cfg2", args2.configs2)
    mod2 = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod2)
    for cls_name in ["ModelParams", "OptimizationParams", "PipelineParams"]:
        if hasattr(mod2, cls_name):
            cls_obj = getattr(mod2, cls_name)
            for k, v in vars(cls_obj).items():
                if not k.startswith("__"):
                    if not hasattr(args2, k) or getattr(args2, k) is None:
                        setattr(args2, k, v)
    args2.model_path = args2.modelpath2
if args3.configs3:
    import importlib.util
    from utils.params_utils import merge_hparams
    spec = importlib.util.spec_from_file_location("_uw_cfg3", args3.configs3)
    mod3 = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod3)
    for cls_name in ["ModelParams", "OptimizationParams", "PipelineParams"]:
        if hasattr(mod3, cls_name):
            cls_obj = getattr(mod3, cls_name)
            for k, v in vars(cls_obj).items():
                if not k.startswith("__"):
                    if not hasattr(args3, k) or getattr(args3, k) is None:
                        setattr(args3, k, v)
